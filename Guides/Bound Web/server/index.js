...
function resetMatchForReplay(room) {
  ...
  room.message = `New game started. ${room.players[room.dealer].name} shuffled. ${room.players[room.cutter].name} must cut the deck.`;
}
...
function finishTrick(room) {
  ...
  if (room.bound) {
    ...
    if (teamOf(winner) !== bidderTeam) {
      ...
      finishMatch(room, winningTeam, `Bound failed. Team ${winningTeam + 1} wins the game.`, bidderTeam, 'bound');
      return;
    }
    ...
    if (room.tricksWon[bidderTeam] === 9) {
      finishMatch(room, bidderTeam, `Bound succeeded. Team ${bidderTeam + 1} wins the game.`, 1 - bidderTeam, 'bound');
      return;
    }
  }
...
function scoreRound(room) {
  ...
  if (room.bound) {
    finishMatch(room, bidderTeam, `Bound succeeded. Team ${bidderTeam + 1} wins the game.`, otherTeam, 'bound');
    return;
  }
...
io.on('connection', socket => {
  ...
  function handleStartNewGame(roomCode) {
    ...
    if (room.players.length !== 4) return socket.emit('errorMessage', 'Need exactly 4 active players to start a new game.');
